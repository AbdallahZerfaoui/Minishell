...
/*   Updated: 2025/02/27 16:14:31 by azerfaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

static void	process_single_heredoc(t_cmd_node *current,
		t_cmd_manager *cmd_manager, t_shell **shell, int idx)
{
	t_token		*file;
	t_heredoc	*heredoc;
	char		*hd_filename;
	...
	while (file)
	{
		if (file->type == HEREDOC && file->next)
		{
			...
			cmd_manager->cmds[idx].hd_filename = heredoc->filename;
		}
		file = file->next;
	}
}
		...
		t_shell **shell)
{
	...
	while (current)
	{
		process_single_heredoc(current, cmd_manager, shell, i);
		i++;
		current = current->next;
	}
}

t_cmd_manager	*initialize_cmd_manager(t_cmd_node *cmds)
{
...
void	setup_command(t_cmd_node *current, t_cmd_manager *cmd_manager,
		t_shell **shell, int idx)
{
	...
	if (current->cmd_array && current->cmd_array[0])
	{
		...
		if (access(current->cmd_array[0], X_OK) == 0 && !custom_cmd_path)
			cmd_manager->cmds[idx].path = ft_strdup(current->cmd_array[0]);
		else
			cmd_manager->cmds[idx].path = custom_cmd_path;
		cmd_manager->cmds[idx].args = current->cmd_array;
	}
	cmd_manager->cmds[idx].fd_in = get_fd_in(current);
	cmd_manager->cmds[idx].fd_out = get_fd_out(current);
}
...
t_cmd_manager	*prepare_execution(t_cmd_node *cmds, t_shell **shell)
{
	...
	cmd_manager = initialize_cmd_manager(cmds);
	if (!cmd_manager->cmds)
		return (NULL);
	...
	while (current)
	{
		setup_command(current, cmd_manager, shell, i);
		current = current->next;
		i++;
	}
